...
public class AccountActivationTokenServiceImpl implements AccountActivationTokenService {
    ...
    public UUID generateToken(String email) {
        if (accountActivationTokenRepository.findByUserEmail(email).isPresent()) {
            ...
            throw new ElException(ElErrorMessage.ACCOUNT_ACTIVATION_TOKEN_ALREADY_EXISTS);
        }
    ...
    public UUID findByEmail(String email) {
        return accountActivationTokenRepository.findByUserEmail(email).map(AccountActivationToken::getToken).orElseThrow(() -> {
            ...
            return new ElException(ElErrorMessage.ACCOUNT_ACTIVATION_TOKEN_NOT_FOUND);
        });
    }
    ...
    public void deleteToken(UUID token) {
        ...
        logger.info("[ACCOUNT ACTIVATION TOKEN] : Account activation token {} successfully deleted", token);
    }

    @Override
    public void deleteByUserEmail(String email) {
        accountActivationTokenRepository.deleteByUserEmail(email);
        logger.info("[ACCOUNT ACTIVATION TOKEN] : Password change token successfully deleted for user {}", email);
    }
}
